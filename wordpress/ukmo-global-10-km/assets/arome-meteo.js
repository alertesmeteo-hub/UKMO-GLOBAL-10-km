(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'ukmog-temp-extreme'; }
        if (value >= 30) { return 'ukmog-temp-hot'; }
        if (value >= 22) { return 'ukmog-temp-warm'; }
        if (value >= 12) { return 'ukmog-temp-mild'; }
        if (value >= 4) { return 'ukmog-temp-cool'; }
        return 'ukmog-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'ukmog-gust-danger'; }
        if (value >= 60) { return 'ukmog-gust-strong'; }
        if (value >= 40) { return 'ukmog-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'ukmog-wind-calm'; }
        if (value >= 80) { return 'ukmog-wind-violent'; }
        if (value >= 60) { return 'ukmog-wind-strong'; }
        if (value >= 40) { return 'ukmog-wind-fresh'; }
        if (value >= 20) { return 'ukmog-wind-moderate'; }
        return 'ukmog-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'ukmog-rain ukmog-rain-intense'; }
        if (value >= 5) { return 'ukmog-rain ukmog-rain-strong'; }
        if (value >= 2) { return 'ukmog-rain ukmog-rain-moderate'; }
        if (value >= 0.5) { return 'ukmog-rain ukmog-rain-light'; }
        return 'ukmog-rain ukmog-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'ukmog-chart-day-band ukmog-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'ukmog-chart-day-strip ukmog-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'ukmog-chart-day-separator ukmog-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'ukmog-chart-day-label ukmog-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'ukmog-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'ukmog-chart-metric';
            item.appendChild(textSpan(metric.label, 'ukmog-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'ukmog-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'ukmog-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'ukmog-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-ukmog-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-ukmog-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'ukmog-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'ukmog-chart-empty'));
            return;
        }
        ...
        svg.classList.add('ukmog-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'ukmog-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'ukmog-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'ukmog-chart-grid ukmog-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'ukmog-chart-axis ukmog-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'ukmog-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'ukmog-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'ukmog-chart-hitpoint ' : 'ukmog-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-ukmog-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'ukmog-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.ukmog-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'ukmog-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'ukmog-chart-axis ukmog-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'ukmog-chart-rain-zero'; }
        if (value >= 15) { return 'ukmog-chart-rain-intense'; }
        if (value >= 5) { return 'ukmog-chart-rain-strong'; }
        if (value >= 2) { return 'ukmog-chart-rain-moderate'; }
        if (value >= 0.5) { return 'ukmog-chart-rain-light'; }
        return 'ukmog-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'ukmog-chart-axis ukmog-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'ukmog-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-ukmog-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'ukmog-chart-line ukmog-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'ukmog-chart-hitpoint ukmog-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-ukmog-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'ukmog-chart-rain-bar ukmog-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var hours = Math.max(1, Math.min(168, parseInt(app.dataset.hours || '168', 10)));
        var timezone = app.dataset.timezone || 'Europe/Paris';
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions UKMO Global';

        var input = app.querySelector('.ukmog-city-input');
        var locateButton = app.querySelector('[data-ukmog-locate]');
        var results = app.querySelector('.ukmog-search-results');
        var status = app.querySelector('.ukmog-search-status');
        var generalBody = app.querySelector('[data-ukmog-body-general]');
        var stormBody = app.querySelector('[data-ukmog-body-storms]');
        var stormSummary = app.querySelector('[data-ukmog-storm-summary]');
        var snowBody = app.querySelector('[data-ukmog-body-snow]');
        var snowSummary = app.querySelector('[data-ukmog-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-ukmog-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-ukmog-panel]'));
        var title = app.querySelector('[data-ukmog-title]');
        var altitudeLine = app.querySelector('[data-ukmog-altitude]');
        var meta = app.querySelector('[data-ukmog-meta]');
        var generated = app.querySelector('[data-ukmog-generated]');
        var stale = app.querySelector('[data-ukmog-stale]');
        var stormTopScroll = app.querySelector('[data-ukmog-top-scroll="storms"]');
        var stormScrollWrap = app.querySelector('[data-ukmog-scroll-wrap="storms"]');
        var stormTable = app.querySelector('.ukmog-storm-table');
        var snowTopScroll = app.querySelector('[data-ukmog-top-scroll="snow"]');
        var snowScrollWrap = app.querySelector('[data-ukmog-scroll-wrap="snow"]');
        var snowTable = app.querySelector('.ukmog-snow-table');
        var chartTemperature = app.querySelector('[data-ukmog-chart-temperature]');
        var chartPressure = app.querySelector('[data-ukmog-chart-pressure]');
        var chartRain = app.querySelector('[data-ukmog-chart-rain]');
        var chartWind = app.querySelector('[data-ukmog-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-ukmog-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-ukmog-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-ukmog-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-ukmog-chart-title-wind]');
        var rainTotal = app.querySelector('[data-ukmog-rain-total]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'ukmog-day-cell ukmog-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'ukmog-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'ukmog-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('ukmog-search-error', Boolean(error));
        }

        function setActiveView(view) {
            tabs.forEach(function (button) {
                var active = button.dataset.ukmogTab === view;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-selected', active ? 'true' : 'false');
            });
            panels.forEach(function (panel) {
                panel.hidden = panel.dataset.ukmogPanel !== view;
            });
            ...
            if (view === 'map' && selectedMapFocus) {
                window.requestAnimationFrame(function () {
                    var mapApp = app.querySelector('[data-ukmogm-app]');
                    if (mapApp) {
                        mapApp.dispatchEvent(new CustomEvent('ukmogm:focus-location', {
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'ukmog-loading ukmog-load-error' : 'ukmog-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('ukmog-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('ukmog-snow-summary-warning', Boolean(error));
            }
        }
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    if (!payload || payload.status !== 'ok' || !payload.departments) {
                        throw new Error('Index national UKMO Global invalide');
                    }
                    indexData = payload;
                    ...
                    meta.textContent = 'Run du ' + run + ' • résolution ' + resolution +
                        ((payload.model && payload.model.storm_diagnostics) ? ' • diagnostics orages + neige UKMO Global' : '');
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'ukmog-hazard ukmog-hazard-' + numeric);
            ...
            if (!forecasts.length) {
                showTableMessage('Aucune échéance UKMO Global disponible pour cette commune.', true);
                return;
            }
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('ukmog-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'ukmog-hour ukmog-hour-' + weekdayToken(date));
                ...
                var windCell = createCell('td', 'Vent', 'ukmog-wind-cell ' + windForceClass(windDisplay));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('ukmog-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'ukmog-hour ukmog-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'ukmog-thunder-risk ukmog-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'ukmog-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'ukmog-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'ukmog-cape-high' : (finite(cape) && cape >= 500 ? 'ukmog-cape-mid' : ''));
                capeCell.textContent = finite(cape) && Number(cape) >= 25 ? formatNumber(cape, 0) + ' J/kg' : '—';
                capeCell.title = 'CAPE instantanée directement produite par UKMO Global ; les valeurs quasi nulles ne sont pas affichées.';
                stormRow.appendChild(capeCell);
                appendNumber(stormRow, 'LCL', value(values, 'lcl_m'), 0, ' m');

                var lightning = value(values, 'lightning_score');
                appendNumber(stormRow, 'Foudre', lightning, 0, '/100', finite(lightning) && lightning >= 60 ? 'ukmog-lightning-high' : '');
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'ukmog-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'ukmog-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'ukmog-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'ukmog-advanced-list ukmog-details-inline';
                ...
                [
                    ...
                    box.className = 'ukmog-advanced-item';
                    box.appendChild(textSpan(item[0], 'ukmog-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'ukmog-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('ukmog-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'ukmog-hour ukmog-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'ukmog-snow-risk ukmog-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'ukmog-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'ukmog-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'ukmog-stick ukmog-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'ukmog-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'ukmog-details-cell ukmog-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'ukmog-details-button';
                ...
                snowAdvanced.className = 'ukmog-snow-profile ukmog-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'ukmog-profile-section';
                    section.appendChild(textSpan(titleText, 'ukmog-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'ukmog-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'ukmog-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'ukmog-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'ukmog-profile-value'));
                ...
                appendProfileSection('Sorties et diagnostics UKMO Global', [
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('ukmog-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • CAPE UKMO Global : risque indicatif, réflectivité indisponible.';
                }
            }
            if (snowSummary) {
                snowSummary.classList.toggle('ukmog-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • cumul neige UKMO Global 10 km.';
                }
            }
            ...
            if (altitudeLine) {
                altitudeLine.textContent = finite(altitude)
                    ? 'Altitude de ' + cityName + ' : ≈ ' + formatNumber(altitude, 0) + ' m (point de grille UKMO Global)'
                    : 'Altitude de ' + cityName + ' : — (relancez le workflow GitHub UKMO Global v1.0.0)';
                altitudeLine.classList.toggle('ukmog-altitude-missing', !finite(altitude));
            }
            selectedMapFocus = {
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'ukmog-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'ukmog-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'ukmog-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'ukmog-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function selectCommune(candidate) {
            ...
            loadDepartment(candidate.codeDepartement)
                .then(function (departmentData) {
                    ...
                    if (!commune) {
                        throw new Error('Commune absente du catalogue UKMO Global');
                    }
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                ...
                button.type = 'button';
                button.className = 'ukmog-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'ukmog-result-name');
        ...
        if (!baseUrl) {
            showTableMessage('Adresse des données UKMO Global non configurée.', true);
            return;
        }
        loadIndex()
            .then(function () {
    ...
    whenReady(function () {
        document.querySelectorAll('[data-ukmog-app]').forEach(initApp);
    });
}());
